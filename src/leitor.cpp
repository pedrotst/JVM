...
int main(int argc, char** argv){
	...
	for(n = 0; n < classF.attributes_count; n++) {
		...
		attribute_element.attribute_name_index = read_word(arquivoJava);

		// Leitura do attribute length
		attribute_element.attribute_length = read_word(arquivoJava);
		...
		if(cp_element.tag == CONSTANT_Utf8) {
			...
			if(!strcmp(cp_element.cp_union.constant_Utf8.bytes, "SourceFile")) {
				...
				attribute_element.attribute_union.attr_SourceFile.sourcefile_index = read_word(arquivoJava);
			...
			if(!strcmp(cp_element.cp_union.constant_Utf8.bytes, "ConstantValue")) {
				...
				attribute_element.attribute_union.attr_ConstantValue.constantvalue_index = read_word(arquivoJava);
